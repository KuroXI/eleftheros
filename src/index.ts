...
readdirSync(`${__dirname}/events`)
	...
	.forEach((file) => {
		...
		try {
			eventFile.once
				? client.once(eventFile.name, (...args) => eventFile.handler(client, ...args))
				: client.on(eventFile.name, (...args) => eventFile.handler(client, ...args));
		} catch (err) {
			logger.error(err);
		}
	});
...
readdirSync(`${__dirname}/commands`)
	...
	.forEach((file) => {
		...
		if ("data" in commandFile && "handler" in commandFile) {
			...
			logger.warn(`${file} is missing a required "data" and "handler" property.`);
		}
	});

process.env.DISCORD_TOKEN ? client.login(process.env.DISCORD_TOKEN) : logger.error(MISSING_DISCORD_TOKEN_MSG);
